...
void flexran::app::management::app_firas::process_retrieve(uint64_t tick,const std::string& id) {           
  ...
  do {
    /* Ask the curl_multi_ which is a multi handle if there are a message from the 
     * individual transfers, and if this message is CURLMSG_DONE, we remove the 
     * easy_handle from the multi_handle */	
